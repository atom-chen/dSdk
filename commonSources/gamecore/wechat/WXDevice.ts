...
export default class WXDevice {
    ...
    static vibrateShort(): void {
        ...
        if (GameValues.getLocalData("canVibrate") === false) {
            console.log("已设置不可震动");
            return;
        }
     ...
     * @param dataCallback 
     */
    static getClipboardData(dataCallback: Function): void {
        console.log("----  WXDevice  ----");
        ...
        try {
            wx.getClipboardData({
                ...
                "success": (res) => {
                    if (dataCallback) {
                        if (res && res.data) dataCallback(res.data);
                        else dataCallback(null)
                    }
                }
            })
        } catch (error) {
     ...
     * @param data 
     */
    static setClipboardData(data: string): void {
        console.log("----  WXDevice  ----");
