...
const TIME_TO_HAVE_ALL_FINGERS_IN_GESTURE = 300;
...
const TAP_TIME_TO_TAP_FROM_START_TO_END = 300;
 ...
 * @type {number}
 */
const DOUBLE_TAP_MAX_TIME = 300;

const DOUBLETAP_TIME_TO_TAP_FROM_START_TO_END = 180;
...
const TIME_TO_CONSIDER_VALID_SWIPE = 1000;
...
class GestureTouchInfo {
    ...
    isValidDistance(d: number) {
        ...
        // console.log("【isValidDistance】", this._endX, this._endY, this.x, this._y)
        // console.log(Math.sqrt(x * x + y * y), d)
 ...
 * @param touches 
 */
export function __doTouches(touches: Array<any>) {
    for (let i = 0; i < touches.length; i += 1) {
        let touch: object = touches[i];
        if (touch["id"] == undefined) {
            if (touch["_id"] != undefined) {
                touch["id"] = touch["_id"];
            } else if (touch["__instanceId"] != undefined) {
                touch["id"] = touch["__instanceId"];
            }
        }
    }
}
...
type GestureManagerTargetEventListener = (e: cc.Event.EventTouch) => void;
...
class GestureRecognizer {
    ...
    addTouchInfo(ti: any) {
        // console.log("[addTouchInfo]", ti);
        this._touchesInfo[this._currentTouchIdCount].initialize(ti["id"], ti["_point"]["x"], ti["_point"]["y"]);
        this._currentTouchIdCount += 1;
    }
    ...
    touchesBegan(e: cc.Event.EventTouch) {
        let touches: Array<any> = e.getTouches();
        __doTouches(touches);
        ...
        //检查是否有新增touch
        //===============================================
        let hasNewTouch:boolean = false;
        for (let i = 0; i < touches.length; i += 1) {
            let touch:any = touches[i];
            const touchInfo = this.getTouchInfoById(touch.id);
            if (!touchInfo) {
                hasNewTouch = true;
                break;
            }
        ...
        if (!hasNewTouch) {
            // console.log("【touchesBegan】没有新增touch", "touches.length=" + touches.length);
            return;
        }
        //===============================================
        ...
        if (this.getCurrentTouchIdCount() < this._fingers) {
            if (this.getGestureElapsedTime() > TIME_TO_HAVE_ALL_FINGERS_IN_GESTURE) {
                // console.log("too much time until i get all the fingers needed.");
                this.failed();
                return;
            }
        }

        if (this.getCurrentTouchIdCount() + e.getTouches().length > this._fingers) {
            // console.log("too many fingers!!", this.getCurrentTouchIdCount(), e.getTouches().length, this._fingers, this._id);
            this.failed();
            return;
        }

        // console.log("[touchesBegan]", touches);
        for (let i = 0; i < touches.length; i += 1) {
            this.addTouchInfo(touches[i]);
        }
    ...
    touchesEnded(e) {
        // console.log("【touchesEnded】", e)
        this.__setStatus(GestureStatus.ST_ENDED);
    }

    touchesCanceled(e) {
        // console.log("【touchesCanceled】", e)
...
class GR_Tap extends GestureRecognizer {
    ...
    touchesMoved(e: cc.Event.EventTouch) {
        ...
        for (let i = 0; i < touches.length; i += 1) {
            ...
            const indexId = this.getTouchIdPositionInCaptureArray(touches[i]["id"]);
            if (-1 != indexId) {

                const touchInfo = this._touchesInfo[indexId];

                const offsetX = Math.abs(touchInfo.x - touches[i]["_point"].x);
                const offsetY = Math.abs(touchInfo.y - touches[i]["_point"].y);
    ...
    touchesEnded(e: cc.Event.EventTouch) {
        let touches = e.getTouches();
        __doTouches(touches);
        ...
        for (let i = 0; i < touches.length; i += 1) {
            this.clearTouchId(touches[i]["id"]);
        }
...
class GR_Swipe extends GestureRecognizer {
    ...
    touchesEnded(e: cc.Event.EventTouch) {
        ...
        // console.log("【touchesEnded】", "touches.length=" + touches.length)

        if (this.getCurrentTouchIdCount() !== this._fingers) {
            // console.log("this.getCurrentTouchIdCount()", this.getCurrentTouchIdCount(), this._fingers)
            this.failed();
            return;
        }

        if (this.getGestureElapsedTime() > TIME_TO_CONSIDER_VALID_SWIPE) {
            // console.log("this.getGestureElapsedTime()", this.getGestureElapsedTime(), TIME_TO_CONSIDER_VALID_SWIPE)
            this.failed();
            return;
        }
        ...
        for (let i = 0; i < touches.length; i += 1) {
            ...
            if (touchInfo) {
                touchInfo.setEndPosition(touch["_point"].x, touch["_point"].y);
        ...
        if (this.allCapturedTouchIdsReleased()) {
            ...
            let swipe_dir = this.__getSwipeDirection(this._touchesInfo[0]);
    ...
    __getSwipeDirection(touchInfo: GestureTouchInfo): string {
        ...
        if (angle < Math.PI / 2 && angle < SWIPE_RECOGNIZER_ANGLE_THRESHOLD) {
...
class GR_Pinch5 extends GestureRecognizer {
    ...
    touchesMoved(e: cc.Event.EventTouch) {
        ...
        if (this.getCurrentTouchIdCount() !== this._fingers) {
            // console.log("【Failed】this.getCurrentTouchIdCount() !== this._fingers")
            // console.log(this.getCurrentTouchIdCount(), this._fingers)
            this.failed();
            return;
        }
    ...
    touchesEnded(e: cc.Event.EventTouch) {
        ...
        if (this.getCurrentTouchIdCount() !== this._fingers) {
            // console.log("【Failed】this.getCurrentTouchIdCount() !== this._fingers2")
            // console.log(this.getCurrentTouchIdCount(), this._fingers)
...
class GR_PinchZoom extends GestureRecognizer {
    ...
    touchesBegan(e: cc.Event.EventTouch) {
        // console.log("1【touchesBegan】this.getStatus()=" + this.getStatus(), GestureStatus.ST_BEGAN)
        super.touchesBegan(e);

        // console.log("2【touchesBegan】this.getStatus()=" + this.getStatus(), GestureStatus.ST_BEGAN)
    ...
    touchesMoved(e: cc.Event.EventTouch) {
        ...
        if (this.getCurrentTouchIdCount() !== this._fingers) {
            // console.log("【touchesMoved failed】this.getCurrentTouchIdCount() !== this._fingers", this.getCurrentTouchIdCount(), this._fingers)
            // this.failed();
            return;
        }
        ...
        if (!this._started) {
            if (this._gestureManager.hasGesture(GestureName.LONGTAP) && this.getGestureElapsedTime() > TIME_TO_START_PINCH_ZOOM) {
                // console.log("2【touchesMoved failed】this.getGestureElapsedTime() > TIME_TO_START_PINCH_ZOOM", this.getGestureElapsedTime(), TIME_TO_START_PINCH_ZOOM)
    ...
    touchesEnded(e) {
        if (!this.allCapturedTouchIdsReleased()) {
            // console.log("1【touchesEnded failed】!this.allCapturedTouchIdsReleased()", this.allCapturedTouchIdsReleased())
            this.failed();
        } else {
            super.touchesEnded(e);
        }
...
export class GestureManager {
    ...
    __setTarget(target) {
        if (this._target) {
            this._target.off(cc.Node.EventType.TOUCH_START, this._touchStartFn, this);
            this._target.off(cc.Node.EventType.TOUCH_MOVE, this._touchMoveFn, this);
            this._target.off(cc.Node.EventType.TOUCH_END, this._touchEndFn, this);
            this._target.off(cc.Node.EventType.TOUCH_CANCEL, this._touchEndFn, this);
        ...
        if (this._target) {
            this._target.on(cc.Node.EventType.TOUCH_START, this._touchStartFn, this);
            this._target.on(cc.Node.EventType.TOUCH_MOVE, this._touchMoveFn, this);
            this._target.on(cc.Node.EventType.TOUCH_END, this._touchEndFn, this);
            this._target.on(cc.Node.EventType.TOUCH_CANCEL, this._touchEndFn, this);
        }
    ...
    __touchStartHandler(e: cc.Event.EventTouch) {
        // e.preventDefault();
        let touches: Array<any> = e.getTouches();
        __doTouches(touches);
        ...
        for (let i = 0; i < touches.length; i += 1) {
            const id = touches[i]["id"];
            // console.log("\t touch id is " + id);
    ...
    __touchMoveHandler(e: cc.Event.EventTouch) {
        // e.preventDefault();
        let touches: Array<any> = e.getTouches();
        __doTouches(touches);
    ...
    __touchEndHandler(e: cc.Event.EventTouch) {
        let touches: Array<any> = e.getTouches();
        __doTouches(touches);
    ...
    __touchCancelHandler(e: cc.Event.EventTouch) {
        let touches = e.getTouches();
        __doTouches(touches);
    ...
    private __onGestures: Array<GestureName> = [];
