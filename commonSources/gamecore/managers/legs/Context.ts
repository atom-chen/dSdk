...
export default class Context {
     ...
     */
    public mapEvent(eventName:string, commandClass:any, priority:number = 10000):void {
        if (this._eventMap[eventName] == undefined) {
            this._eventMap[eventName] = [];
            GameManager.eventManager.addEventListener(eventName, this.eventsHandler, this, priority);
        }
     ...
     */
    public unmapEvent(eventName:string, commandClass = null):void {
        let commandClasses:Array<any> = this._eventMap[eventName];
            ...
            if (commandClass) {
                ...
                if (index >= 0) {
                    ...
                    if (commandClasses.length == 0) {
                        ...
                        GameManager.eventManager.removeEventListener(eventName, this.eventsHandler, this);
                    }
                }
            } else {
                //取消绑定所有该事件的命令
     ...
     * 事件管理器
     */
    private eventsHandler(evt:EventData):void {
