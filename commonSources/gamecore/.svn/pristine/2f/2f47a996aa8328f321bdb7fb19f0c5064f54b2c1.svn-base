...
@ccclass
export default class Comp_LoadingSceneMain extends cc.Component {
    ...
    start() {
        ...
        GameManager.eventManager.addEventListener(GameCoreEventNames.RES_PROGRESS, this.onProgress, this);
     ...
     * @param e 
     */
    private onProgress(e: EventData): void {
        if (this.progress) {
            this.progress.progress = e.data.min / e.data.max
        }
        if (this.titleNode) {
    ...
    public doPreDestory(duration: number = 0): void {
        ...
        GameManager.eventManager.removeEventListener(GameCoreEventNames.RES_PROGRESS, this.onProgress, this);
