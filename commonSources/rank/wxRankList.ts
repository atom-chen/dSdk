...
export default class wxRankList extends cc.Component {
    ...
    hideFriendRank(){
        if(this.rankListNode.active == false){
            return;
        }
        this.onDisable();
        this.postMessage(Consts.DomainAction.Clear);
        this.rankListNode.active = false;
    }
    ...
    uploadLevelOpen(level){
        ...
        let v = '{"wxgame":{"score":' + level + ', "update_time":' + timestamp + '}}'
        window["wx"].setUserCloudStorage({
            KVDataList: [
                { key: Consts.OpenDataKeys.LevelKey, value: v },
            ],
            success: (res) => {
                console.log("uploadScore success:res=>", res)
            },
    ...
    */
    share(args) {
        if (!window.window["wx"]) return
        if(!args) args={}
        args.imageUrl = args.imageUrl || "http://xxx/cxxx.jpg"
